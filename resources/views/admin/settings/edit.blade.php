<x-admin-layout>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700" x-data="{ type: '{{ old('type', $setting->type) }}' }">
            <form action="{{ route('settings.update', $setting) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="key" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Key Identifier</label>
                    <input type="text" name="key" id="key" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('key', $setting->key) }}" required>
                    @error('key')
                ...
                <div class="mb-4" x-show="type === 'text'">
                    ...
                    <input type="text" name="value" id="value_text" :disabled="type !== 'text'" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('value', $setting->type === 'text' ? $setting->value : '') }}">
                </div>
                ...
                <div class="mb-4" x-show="type === 'textarea'" style="display: none;">
                    ...
                    <textarea name="value" id="value_textarea" :disabled="type !== 'textarea'" rows="4" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('value', $setting->type === 'textarea' ? $setting->value : '') }}</textarea>
                </div>
