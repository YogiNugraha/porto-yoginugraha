<x-admin-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 border-b border-slate-200" x-data="{ type: '{{ old('type', 'text') }}' }">
            <form action="{{ route('settings.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-4">
                    <label for="key" class="block text-sm font-medium text-slate-700">Key Identifier (e.g. hero_title)</label>
                    <input type="text" name="key" id="key" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm" value="{{ old('key') }}" required>
                    @error('key')
                ...
                <div class="mb-4" x-show="type === 'text'">
                    ...
                    <input type="text" name="value" id="value_text" :disabled="type !== 'text'" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm" value="{{ old('value') }}">
                </div>
                ...
                <div class="mb-4" x-show="type === 'textarea'" style="display: none;">
                    ...
                    <textarea name="value" id="value_textarea" :disabled="type !== 'textarea'" rows="4" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm">{{ old('value') }}</textarea>
                </div>
